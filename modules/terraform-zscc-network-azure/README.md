...
## Private Endpoint / Function App VNet Integration Subnets

Set `private_endpoint_enabled = true` to create a dedicated subnet (`private_endpoint_subnet_id` output) for Key Vault/Storage Account Private Endpoints, consumed by `terraform-zscc-keyvault-azure`'s `private_endpoint_subnet_id` and `terraform-zscc-function-app-azure`'s `storage_private_endpoint_subnet_id` variables.

Set `function_app_vnet_integration_enabled = true` to create a subnet delegated to `Microsoft.Web/serverFarms` (`function_app_subnet_id` output) for Function App regional VNet Integration, consumed by `terraform-zscc-function-app-azure`'s `vnet_integration_subnet_id` variable.
...
| <a name="input_function_app_subnet"></a> [function\_app\_subnet](#input\_function\_app\_subnet) | Function App VNet Integration Subnet to create in VNet. This is only required if you want to override the default subnet that this code creates via network\_address\_space variable. Only used when function\_app\_vnet\_integration\_enabled is true. | `string` | `null` | no |
| <a name="input_function_app_vnet_integration_enabled"></a> [function\_app\_vnet\_integration\_enabled](#input\_function\_app\_vnet\_integration\_enabled) | Configure a subnet delegated to Microsoft.Web/serverFarms for Function App regional VNet Integration if set to true | `bool` | `false` | no |
...
| <a name="input_private_endpoint_enabled"></a> [private\_endpoint\_enabled](#input\_private\_endpoint\_enabled) | Configure a dedicated subnet for Key Vault/Storage Account Private Endpoints if set to true | `bool` | `false` | no |
| <a name="input_private_endpoint_subnet"></a> [private\_endpoint\_subnet](#input\_private\_endpoint\_subnet) | Private Endpoint Subnet to create in VNet. This is only required if you want to override the default subnet that this code creates via network\_address\_space variable. Only used when private\_endpoint\_enabled is true. | `string` | `null` | no |
...
| <a name="output_function_app_subnet_id"></a> [function\_app\_subnet\_id](#output\_function\_app\_subnet\_id) | Function App VNet Integration Subnet ID |
...
| <a name="output_private_endpoint_subnet_id"></a> [private\_endpoint\_subnet\_id](#output\_private\_endpoint\_subnet\_id) | Private Endpoint Subnet ID |
