...
* Optional Terraform automation for the Azure Managed Identity and Key Vault prerequisites documented in the root README.md Azure Requirements section (previously manual-only, steps 4/5/9). Delivered as standalone child modules only — not yet wired into any examples/ root module.
    - add: module terraform-zscc-keyvault-azure to optionally create an RBAC-authorization-only Azure Key Vault, populate it with the `api-key`/`username`/`password` Zscaler Cloud Connector credential secrets, and grant the built-in "Key Vault Secrets User" role to a caller-supplied principal
    - add: terraform-zscc-identity-azure support for optionally creating (instead of only looking up) the CC VM and Function App user-assigned managed identities, plus an optional "Network Contributor" (or custom role) role assignment at a caller-supplied scope
    - refactor: terraform-zscc-identity-azure data source lookups are now count-gated to support the new create-vs-lookup toggle; existing module outputs and behavior are unchanged when new variables are left at their (backward-compatible) defaults
* Optional Private Endpoint network lockdown for the Key Vault (terraform-zscc-keyvault-azure) and Function App Storage Account (terraform-zscc-function-app-azure), plus regional VNet Integration for the Function App itself, so both PaaS dependencies can be made reachable only from the Cloud Connector VNet. All new module variables default to false/backward-compatible.
    - add: terraform-zscc-network-azure support for a dedicated Private Endpoint subnet (private_endpoint_enabled) and a Microsoft.Web/serverFarms-delegated subnet for Function App VNet Integration (function_app_vnet_integration_enabled)
    - add: terraform-zscc-keyvault-azure support for a Key Vault Private Endpoint (private_endpoint_enabled), including automatic privatelink.vaultcore.azure.net Private DNS Zone creation/VNet-link or a BYO zone via existing_private_dns_zone_id
    - add: terraform-zscc-function-app-azure support for Storage Account network rules (storage_public_network_access_enabled/storage_network_rules_default_action) and per-subresource Private Endpoints (storage_private_endpoint_enabled) covering blob/file/queue/table — Azure only permits one subresource per Storage Account Private Endpoint, so one is created per requested subresource
    - add: terraform-zscc-function-app-azure support for Function App regional VNet Integration (vnet_integration_enabled), including the WEBSITE_CONTENTOVERVNET app setting required when the content share storage account sits behind a Private Endpoint. Requires asp_sku_name = "EP1" — documented in the module README, not enforced (Y1/FC1/B1 do not support regional VNet Integration)
    - add: greenfield example base_cc_vmss now wires terraform-zscc-keyvault-azure (creating and populating the Key Vault, replacing the previous manually-provisioned azure_vault_url input) end-to-end with the new Private Endpoint/VNet Integration support, so Cloud Connectors and the Function App both reach the Key Vault and Storage Account only via Private Endpoint within the VNet
    - fix: base_cc_vmss example no longer references the deleted terraform-zscc-public-lb-azure module (dangling module source since that module's removal); removed the unused public_lb_deploy toggle and its downstream wiring
